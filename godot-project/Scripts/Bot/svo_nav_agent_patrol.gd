...
func _ready():
	...
	if patrol_points.size() > 0:
		navigate_to_point(current_patrol_index)
...
func navigate_to_point(index):
	if index >= patrol_points.size():
		index = 0
		current_patrol_index = 0
	var target_node = patrol_points[index]
	if target_node:
		find_path_multi_thread(target_node.position)
...
func find_path_multi_thread(pos: Vector3):
	current_target_index = 0
	path.clear()
	var data = [self.position, pos, agent_r_temp]
	var direct_path = svo.direct_path_check(self.position, pos, agent_r_temp)
	if not direct_path:
