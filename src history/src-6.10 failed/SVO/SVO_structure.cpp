...
OctreeNode::OctreeNode() : isLeaf(true), isHomogeneous(false), voxel(nullptr) {
    ...
    for (int i = 0; i < 6; ++i) {  // 假设 NUM_NEIGHBORS 是邻居数组的大小
...
SparseVoxelOctree::SparseVoxelOctree(int max_depth)
    : maxDepth(max_depth){
    //root = new OctreeNode();
}
...
void SparseVoxelOctree::update(OctreeNode* node, int x, int y, int z, int depth, bool isSolid) {
    ...
    if (depth == maxDepth) {
        if (!node->voxel) {
            node->voxel = new Voxel(isSolid); // 如果体素不存在，则创建新体素
        }
        else {
    ...
    // 检查并更新节点的同质性状态
    updateHomogeneity(node);*/
}

void SparseVoxelOctree::updateHomogeneity(OctreeNode* node) {
    /*bool allHomogeneous = true;
    ...
    if (allHomogeneous) {
        ...
        node->isLeaf = true;
        node->isHomogeneous = true;
        node->voxel = new Voxel(firstSolid); // 创建新的体素表示所有子节点的统一状态
    }*/
}
